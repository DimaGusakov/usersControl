...
const updateLocalStorage = () => localStorage.setItem("users", JSON.stringify(users))
  ...
  if (usersData) {
    try {
      ...
      return parsedUsers.map(user => {
        const UserClass = getCorrectUserClass(user.role)
        if (user.role === "admin") {
        ...
        else return new UserClass(user.id, user.name, user.email, user.role)
      })
    } 
    catch (e) {
    ...
    tableContent = `
      <tr>
        <td colspan="5" class="no-data">
          ${arr === users ? "Нет пользователей" : "Пользователи не найдены"}
        </td>
      </tr>
    `
  } 
  else arr.forEach(user => (tableContent += user.renderRow()));
...
closeBtn.addEventListener("click",() => (userFormModal.style.display = "none"))
...
userForm.addEventListener("submit", e => {
  ...
  if (userId) {
    ...
    if (userIndex !== -1) {
      const UserClass = getCorrectUserClass(role)
      if (role === "admin") {
        users[userIndex] = new UserClass(+userId, name, email, role, [
          "create_user",
          "edit_user",
          "delete_user",
          "view_user_list",
      ...
      else users[userIndex] = new UserClass(+userId, name, email, role)
    }
  } 
  else {
    ...
    else users.push(new UserClass(newUserId, name, email, role))
  }
  render(users)
  userFormModal.style.display = "none"
});

function getCorrectUserClass(role) {
  switch (role) {
    case "admin":
      return AdminUser
    case "regular":
